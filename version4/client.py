...
from game import resources, gui
...
def on_key_press(symbol, modifiers):
	...
	if symbol == key.SPACE:
		...
		elif game_state == SharedVar.state['TRANSITION']:
			clientsocket.close()
			event_loop.exit()
			pyglet.app.exit()
...
start_label = pyglet.text.Label('Start Game!',
...
host_label = pyglet.text.Label('host',
						font_name = 'Liberation Serif',
						font_size = 28,
						x = game_window.width//2 - 200,
						y = game_window.height//2 - 60,
						anchor_x = 'center',
						anchor_y = 'center',
						batch=start_batch
						)

port_label = pyglet.text.Label('port',
						font_name = 'Liberation Serif',
						font_size = 28,
						x = game_window.width//2 - 200,
						y = game_window.height//2 - 110,
						anchor_x = 'center',
						anchor_y = 'center',
						batch=start_batch
						)

username_label = pyglet.text.Label('username',
						font_name = 'Liberation Serif',
						font_size = 28,
						x = game_window.width//2 - 200,
						y = game_window.height//2 - 160,
						anchor_x = 'center',
						anchor_y = 'center',
						batch=start_batch
						)

get_host = gui.TextWidget('',
...
def on_draw():
	...
	if game_state == SharedVar.state['END'] or game_state == SharedVar.state['TRANSITION']:
		pyglet.sprite.Sprite(img=resources.sprite['game_over'],x=0,y=0).draw()

	elif game_state == SharedVar.state['START']:
		pyglet.sprite.Sprite(img=resources.sprite['start'],x=0,y=0).draw()
		start_batch.draw()
...
def update(dt):
	...
	if game_state == SharedVar.state['SETUP']:
		...
		player1 = 6#data['player1']
		player2 = 4#data['player2']
		print "GAME OVER"
