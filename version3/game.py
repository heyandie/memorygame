...
This is the edited code for the game. Add menu and extra frills here.
(To be integrated with server and client files soon.)

Things to add:
	[ ] Ask user for host, port, username (optional)
	[ ] Title Screen/Menu
	[ ] Show own score
	[ ] Option to hide/show score to opponent
	[ ] Option to quit (other player should end game and quit or wait for another player to connect)
	[ ] Notify other player if disconnected

Requirements:
	[ ] Client identifier (distinct)
	[ ] Optional: username for players (may have duplicate)
	[ ] Receive messages from others
	[ ] Send messages to all or selected users
	[ ] Option to share other kinds of info (score?)
	[ ] Voluntary disconnect
	[ ] Notify if someone disconnects
